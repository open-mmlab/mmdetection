...
from mmdet.models.dense_heads import (AnchorHead, FCOSHead, FSAFHead, PAAHead,
                                      RetinaHead, YOLOV3Head, paa_head)
from tests.test_onnx.utils import verify_model
...
class AnchorHeadTest(nn.Module):
    ...
    def __init__(self,
        ...
        self.head = test_head(num_classes=num_classes, 
                            in_channels=in_channels, 
                            test_cfg=test_cfg, **head_cfg)
    ...
    def forward(self,feat):
        ...
        if(self.test_step == 2): # get_anchor test
            ...
            anchors_outs = self.head.get_anchors(featmap_sizes, self.img_metas, device="cpu")
            ret_list = []
        ...
        if(self.test_step == 3): # get_bboxes test
            ...
            featmap_sizes = [cls_scores[i][0].shape[-2:] for i in range(num_levels)]
            cls_score_list = [
        ...
        #--------------------------------------------
        result_list = self.head.get_bboxes(cls_scores, bbox_preds, self.img_metas)
...
def retinanet_config(test_step_name):
    ...
    model = AnchorHeadTest(RetinaHead, head_cfg, test_cfg, img_metas, 
            test_step=test_step_names[test_step_name], num_classes=4, in_channels=1)
    model.eval()

    feat = [torch.rand(1, 1, 256 // (2**(i + 2)), 256 // (2**(i + 2)))
    ...
    def forward(self,feat):
        ...
        if(self.test_step == 3): # get_bboxes test
            ...
            if self.with_nms:
                ...
                outs = self._get_bboxes_single(pred_maps_list,
                                                scale_factor, None, self.with_nms)
...
def yolo_config(test_step_name):
    ...
    model = YoloV3HeadTest(YOLOV3Head, head_cfg, test_cfg, img_metas,
            in_channels=[1, 1, 1],
