...
def test_retinanet_head_get_bboxes():
    ...
    retina_model.bbox_post_processes = nn.Identity()
    retina_model.get_bboxes = partial(
        retina_model.get_bboxes, img_metas=img_metas, cfg=cfg)
...
def test_fcos_head_get_bboxes():
    ...
    cfg = mmcv.ConfigDict(dict(deploy=dict(with_nms=False)))
    fcos_model.bbox_post_processes = nn.Identity()
    fcos_model.get_bboxes = partial(
        fcos_model.get_bboxes, img_metas=img_metas, cfg=cfg)
...
def test_fsaf_head_get_bboxes():
    ...
    bboxes = feats[5:]
    cfg = mmcv.ConfigDict(dict(deploy=dict(with_nms=False)))
    fsaf_model.bbox_post_processes = nn.Identity()
    fsaf_model.get_bboxes = partial(
        fsaf_model.get_bboxes, img_metas=img_metas, cfg=cfg)
