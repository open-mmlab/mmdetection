...
def convert_result_list(outputs):
    ...
    Args:
        output (list(Tensor) | tuple(list(Tensor) |
            list(obj:`InstanceResults`)...):
        the outputs in torch env, maybe containing nested
        structures such as list or tuple.
    ...
    if isinstance(outputs, torch.Tensor):
        return [outputs]
    if isinstance(outputs[0], Results):
        det_results = []
        for results in outputs:
            if 'score_factors' in results:
                results.scores = results.scores * results.score_factors[:,
                                                                        None]
            det_results.append((results.bboxes, results.scores))
        outputs = det_results
    ret = []
    for sub in outputs:
