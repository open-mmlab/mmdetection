...
import sys
import unittest
from common import collect_ap, replace_text_in_file
...
class PublicModelsTestCase(unittest.TestCase):
    root_dir = '/home/yuchunli/_DATASET'
    coco_dir = osp.join(root_dir, 'coco')
    ...
    def setUpClass(cls):
        ...
        cls.annotation_file = osp.join(
            cls.coco_dir,
            f'annotations/instances_val2017_short_{cls.shorten_to}.json')
        ...
        for expected, actual, m in zip(reference_ap, ap, metrics):
            if abs(actual - expected) > thr:
                raise AssertionError(
                    f'{m}: {expected} (expected) - {thr} (threshold) > {actual}'
                )
    ...
    def domain_check_for_custom_operations(self, config_dir):
        ...
        from mmdet.utils.deployment.operations_domain import \
    ...
    def run_openvino_export_test(self,
                                 config_path,
                                 snapshot,
                                 metrics=('bbox', ),
                                 thr=0.02,
        ...
        print(
            '\n\ntesting OpenVINO export ' +
            '(--alt_ssd_export)' if alt_ssd_export else '' + config_path,
            file=sys.stderr)
        name = config_path.replace('configs/', '')[:-3]
        test_dir = osp.join(
            self.root_dir, name,
            'openvino_alt_ssd_export' if alt_ssd_export else 'openvino_export')
        log_file, target_config_path = self.prerun(config_path, test_dir)
    ...
    def run_onnx_export_test(self,
        ...
        with open(log_file, 'w') as log_f:
            ...
            try:
                run([
                    'python', 'tools/export.py', target_config_path, snapshot,
                    test_dir, 'onnx'
                ],
                    stdout=log_f,
                    stderr=PIPE,
            ...
            try:
                run([
                    'python', 'tools/test_exported.py', target_config_path,
                    osp.join(test_dir, 'model.onnx'), '--out', 'res.pkl',
                    '--eval', *metrics
                ],
                    stdout=log_f,
                    stderr=PIPE,
    ...
    def test_pytorch_fcos__fcos_x101_64x4d_fpn_gn_head_mstrain_640_800_4x2_2x_coco(
            self):
    ...
    def test_openvino_fcos__fcos_x101_64x4d_fpn_gn_head_mstrain_640_800_4x2_2x_coco(
            self):
        origin_config = 'configs/fcos/fcos_x101_64x4d_fpn_gn-head_mstrain_640-800_2x_coco.py'
        url = 'https://download.openmmlab.com/mmdetection/v2.0/' \
              'fcos/fcos_x101_64x4d_fpn_gn-head_mstrain_640-800_4x2_2x_coco/' \
    ...
    #     url = 'https://download.openmmlab.com/open-mmlab/mmdetection/models/htc/' \
    ...
    def test_openvino_alt_ssd_ssd300_coco(self):
        ...
        url = 'https://download.openmmlab.com/mmdetection/v2.0/' \
              'ssd/ssd300_coco/ssd300_coco_20200307-a92d2092.pth'
        self.run_openvino_export_test(
            origin_config, self.download_if_not_yet(url), alt_ssd_export=True)
    ...
    def test_onnx_fcos__fcos_x101_64x4d_fpn_gn_head_mstrain_640_800_4x2_2x_coco(
            self):
        origin_config = 'configs/fcos/fcos_x101_64x4d_fpn_gn-head_mstrain_640-800_2x_coco.py'
        url = 'https://download.openmmlab.com/mmdetection/v2.0/' \
              'fcos/fcos_x101_64x4d_fpn_gn-head_mstrain_640-800_4x2_2x_coco/' \
    ...
    #     url = 'https://download.openmmlab.com/mmdetection/v2.0/htc/' \
