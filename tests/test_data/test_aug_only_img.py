...
def test_color():
    # test assertion with invalid value of level
    with pytest.raises(AssertionError):
        transform = dict(type='Color', level=-1)
        build_from_cfg(transform, PIPELINES)

    # test assertion with invalid type of level
    with pytest.raises(AssertionError):
        transform = dict(type='Color', level=(0.5, ))
        build_from_cfg(transform, PIPELINES)

    # test assertion with invalid value of prob
    with pytest.raises(AssertionError):
        transform = dict(type='Color', level=2, prob=1.2)
        build_from_cfg(transform, PIPELINES)

    # test case when no modification on image (factor=1.0)
    results = construct_toy_data()
    # set level to be 5, the corresponding factor is 1.0
    transform = dict(type='Color', level=5, prob=1.0)
    color_module = build_from_cfg(transform, PIPELINES)
    results_transformed = color_module(copy.deepcopy(results))
    assert np.equal(results['img'], results_transformed['img']).all()

    # case when no transformation
    transform = dict(type='Color', level=5, prob=0.)
    color_module = build_from_cfg(transform, PIPELINES)
    color_module(copy.deepcopy(results))

    # case when facotr > 1
    transform = dict(type='Color', level=10, prob=1.)
    color_module = build_from_cfg(transform, PIPELINES)
    color_module(copy.deepcopy(results))


def test_sharpness():
    # test ValueError with invalid type of ksize
    with pytest.raises(ValueError):
        transform = dict(type='Sharpness', level=1, ksize=1.2)
        build_from_cfg(transform, PIPELINES)

    # test assertion with invalid number of elements in ksize
    with pytest.raises(AssertionError):
        transform = dict(type='Sharpness', level=1, ksize=(3, 3, 3))
        build_from_cfg(transform, PIPELINES)

    # test assertion with invalid value of ksize (not positive)
    with pytest.raises(AssertionError):
        transform = dict(type='Sharpness', level=2, ksize=0)
        build_from_cfg(transform, PIPELINES)

    # test assertion with invalud valud of ksize (not all odd)
    with pytest.raises(AssertionError):
        transform = dict(type='Sharpness', level=2, ksize=(3, 4))
        build_from_cfg(transform, PIPELINES)

    # test assertion with invalid type of sigmaX
    with pytest.raises(AssertionError):
        transform = dict(type='Sharpness', level=1, sigmaX=(0.5, ))
        build_from_cfg(transform, PIPELINES)

    # test assertion with invalid value of prob
    with pytest.raises(AssertionError):
        transform = dict(type='Sharpness', level=2, prob=1.3)
        build_from_cfg(transform, PIPELINES)

    # test case when no modification on image (factor=1.0)
    results = construct_toy_data()
    # set level to be 5, the corresponding factor is 1.0
    transform = dict(type='Sharpness', level=5, prob=1.)
    sharpness_module = build_from_cfg(transform, PIPELINES)
    results_transformed = sharpness_module(results)
    assert np.equal(results['img'], results_transformed['img']).all()

    # case when no transformation
    transform = dict(type='Sharpness', level=5, prob=0.)
    sharpness_module = build_from_cfg(transform, PIPELINES)
    sharpness_module(copy.deepcopy(results))


def test_equalize():
    # test assertion with invalid value of prob
    with pytest.raises(AssertionError):
        transform = dict(type='Equalize', prob=1.2)
        build_from_cfg(transform, PIPELINES)

    results = construct_toy_data()
    # equalize the image
    transform = dict(type='Equalize', prob=1.)
    equalize_module = build_from_cfg(transform, PIPELINES)
    equalize_module(copy.deepcopy(results))
