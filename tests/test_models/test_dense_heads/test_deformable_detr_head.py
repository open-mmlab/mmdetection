...
def test_detr_head_loss():
    ...
    self = DeformableDETRHead(**config)
    self.init_weights()
    feat = [
        torch.rand(1, 256, 16, 16),
        torch.rand(1, 256, 8, 8),
        torch.rand(1, 256, 4, 4),
        torch.rand(1, 256, 2, 2)
    ]
    cls_scores, bbox_preds, enc_class, enc_coord = self.forward(
        feat, img_metas)
    # Test that empty ground truth encourages the network to predict background
    gt_bboxes = [torch.empty((0, 4))]
    gt_labels = [torch.LongTensor([])]
    gt_bboxes_ignore = None
    empty_gt_losses = self.loss(cls_scores, bbox_preds, enc_class, enc_coord,
    ...
    one_gt_losses = self.loss(cls_scores, bbox_preds, enc_class, enc_coord,
                              gt_bboxes, gt_labels, img_metas,
                              gt_bboxes_ignore)
    ...
    # test inference mode
    self.get_bboxes(
        cls_scores, bbox_preds, enc_class, enc_coord, img_metas, rescale=True)
