...
from mmdet.models.dense_heads import (AnchorHead, CenterHead, CornerHead,
                                      FCOSHead, FSAFHead, GuidedAnchorHead,
                                      PAAHead, SABLRetinaHead, TransformerHead,
...
def test_center_head_loss():
    ...
    test_cfg = dict(
        ...
        max_per_img=100)
    self = CenterHead(num_classes=4, in_channels=1, test_cfg=test_cfg)
    ...
    empty_gt_losses = self.loss(center_out, wh_out, offset_out, gt_bboxes,
                                gt_labels, img_metas, gt_bboxes_ignore)
    loss_center = empty_gt_losses['loss_center']
    ...
    one_gt_losses = self.loss(center_out, wh_out, offset_out, gt_bboxes,
                              gt_labels, img_metas, gt_bboxes_ignore)
    loss_center = one_gt_losses['loss_center']
...
def test_center_head_encode_and_decode_heatmap():
    """Tests corner head generating and decoding the heatmap."""
    s = 256
    img_metas = [{
        'img_shape': (s, s, 3),
    ...
    gt_bboxes_ignore = [torch.empty((0, 4))]
    gt_labels = [torch.LongTensor([1, 1, 2])]

    self = CenterHead(num_classes=4, in_channels=1, test_cfg=test_cfg)
    self.feat_shape = (1, 1, s // 4, s // 4)
    targets = self.get_targets(gt_bboxes, gt_labels, img_metas,
                               gt_bboxes_ignore)
    center_target = targets[0]['target']
    wh_target = targets[1]['target']
    offset_target = targets[2]['target']
