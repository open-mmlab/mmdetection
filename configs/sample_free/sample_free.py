...
class SampleFree(object):
    """
    sample-free include Bias Initialization, Guided Loss, Class-Adaptive Threshold,
    to keep the origin mmdet code structure, we compute prior and set score_thr
    before training.

    for one-stage, we compute prior based on the assign result, and simply set the this 
    prior for score_thr.
    for two-stage, we compute prior based on the assign result for rpn stage, while for
    roi-stage, there is no need to compute the prior, and the score_thr is simply set 0.001

    more detail information can refer to the code https://github.com/ChenJoya/sampling-free
    and paper https://arxiv.org/pdf/1909.04868.pdf
    ...
    def compute_prior(self, cfg):
        ...
        #model
        #Note: before build model, we need get the model config without sample-free
        ...
        for img_data in tqdm(dataloader):
            img_metas, img, gt_bboxes = img_data['img_meta'].data[0], img_data['img'].data[0], img_data['gt_bboxes'].data[0]
            h, w = img.shape[-2:]
            featmap_sizes = [(int(ceil(h / stride)), int(ceil(w / stride)))  for stride in strides]

            anchors, valid_flags = getattr(model, head).get_anchors(featmap_sizes, img_metas, device=self.device)
            anchors = [torch.cat(anchors_per_image, 0) for anchors_per_image in anchors]
            valid_flags = [torch.cat(valid_flags_per_image, 0) for valid_flags_per_image in valid_flags]

            for anchor, valid_flag, gt_bbox, img_meta in zip(anchors, valid_flags, gt_bboxes, img_metas):
                inside_flag = anchor_inside_flags(anchor, valid_flag, img_meta['img_shape'][:2], allowed_border)
    ...
    def request_model_info(self, cfg):
        ...
        neck = cfg.model.neck
        model = f'{detector}_{backbone.type}{backbone.depth}_{neck.type}_{self.dataset_name.upper()}'
        return model
    ...
    def set_score_thr(self):
        ...
        cfg_text = cfg_text.replace(f'score_thr={origin_score_thr}', f'score_thr={score_thr}')

        origin_model_key = re.findall(r'model_key=(.*?),', cfg_text)[0]
        cfg_text = cfg_text.replace(f"model_key={origin_model_key}", f"model_key='{self.model_key}'")
...
def parse_args():
    ...
    parser.add_argument('--detector_type', default='two-stage', help='detector is one-stage or two-stage')
    parser.add_argument('--dataset', default='voc', help='dataset name')
    parser.add_argument('--save_path', default='configs/sample_free/init_prior.json', help='save path of computed prior')
