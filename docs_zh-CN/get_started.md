...
(4) 如果希望在安装 `albumentations` 的同时不安装 `opencv-python-headless`，则可以使用 `pip install albumentations>=0.3.2 --no-binary imgaug,albumentations`。 如果希望使用 `opencv-python-headless` 而不是 `opencv-python`， 则可以简单地使用 `pip install albumentations>=0.3.2` 进行安装。更多细节请参考[官方文档](https://albumentations.ai/docs/getting_started/installation/#note-on-opencv-dependencies)。
