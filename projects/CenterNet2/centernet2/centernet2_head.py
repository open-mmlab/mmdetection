...
class CenterNet2Head(BaseDenseHead):
    ...
    def __init__(self,
        ...
        self.sizes_of_interest = [[0, 80], [64, 160], [128, 320], [256, 640],
                                  [512, 10000000]]
        self.min_radius = 4
        ...
        self.head_configs = {
            'cls': (num_cls_convs, use_deformable),
    ...
    def forward(self, feats):
        ...
        Notice CenterNet head does not use FPN.
        Args:
    ...
    def loss_by_feat(
        ...
        flattened_reg_pred = torch.cat(
            [x.permute(0, 2, 3, 1).reshape(-1, 4) for x in reg_pred_per_level],
            0)
        flattened_agn_hm_pred = torch.cat(
            [x.permute(0, 2, 3, 1).reshape(-1)
             for x in agn_hm_pred_per_level], 0) if self.with_agn_hm else None
        flatten_points = torch.cat(
            [points.repeat(len(batch_img_metas), 1) for points in grids])

        losses = self.compute_losses(pos_inds, reg_targets, flattened_hms,
                                     flattened_reg_pred, flattened_agn_hm_pred,
                                     flatten_points)

        return losses

    def compute_losses(self, pos_inds, reg_targets, flattened_hms, reg_pred,
                       agn_hm_pred, flatten_points):
        '''
        Inputs:
            pos_inds: N
            reg_targets: M x 4
        ...
        pos_decoded_target_preds = distance2bbox(flatten_points_pos,
                                                 reg_targets_pos)
        ...
        agn_heatmap_loss = self.loss_center_heatmap(
            agn_hm_pred, cat_agn_heatmap, pos_inds, avg_factor=num_pos_avg)
    ...
    def get_targets(self, grids, shapes_per_level, gt_bboxes):
        ...
        strides = torch.cat([
            shapes_per_level.new_ones(num_loc_list[l1]) * self.strides[l1]
            for l1 in range(L)
        ]).float()  # M
        reg_size_ranges = \
            torch.cat([
                shapes_per_level.new_tensor(
                    self.sizes_of_interest[l2]
                ).float().view(1, 2).expand(num_loc_list[l2], 2)
                for l2 in range(L)])  # M x 2
        ...
        for i in range(len(gt_bboxes)):  # images
            ...
            l_ = grids[:, 0].view(M, 1) - boxes[:, 0].view(1, N)  # M x N
            t_ = grids[:, 1].view(M, 1) - boxes[:, 1].view(1, N)  # M x N
            r_ = boxes[:, 2].view(1, N) - grids[:, 0].view(M, 1)  # M x N
            b_ = boxes[:, 3].view(1, N) - grids[:, 1].view(M, 1)  # M x N
            reg_target = torch.stack([l_, t_, r_, b_], dim=2)  # M x N x 4
        ...
        for hms_per_level in zip(*flattened_hms):
            hms_level_first.append(torch.cat(hms_per_level, dim=0))
    ...
    def assign_fpn_level(self, boxes):
        ...
        size_ranges = boxes.new_tensor(self.sizes_of_interest).view(
            len(self.sizes_of_interest), 2)  # Lx2
    ...
    def assign_reg_fpn(self, reg_targets_per_im, size_ranges):
        ...
        Inputs:
            ...
            size_ranges: M x 2
        '''
        temp = reg_targets_per_im[:, :, :2] + reg_targets_per_im[:, :, 2:]
        crit = (temp**2).sum(dim=2)**0.5 / 2
    ...
    def predict_by_feat(self,
        ...
        boxlists = multi_apply(self._predict_by_feat_single, grids,
                               agn_hm_pred, reg_pred, self.strides,
                               [None for _ in agn_hm_pred])
    ...
    def _predict_by_feat_single(self,
        ...
        for i in range(N):
            ...
            detections[:, 2] = torch.max(detections[:, 2].clone(),
                                         detections[:, 0].clone() + 0.01)
            detections[:, 3] = torch.max(detections[:, 3].clone(),
                                         detections[:, 1].clone() + 0.01)
    ...
    def nms_and_topK(self, boxlist, max_proposals=-1, nms=True):
        ...
        cfg = self.test_cfg
        _, keep = batched_nms(boxlist[:, :4], boxlist[:, 4].contiguous(),
                              boxlist[:, -1], cfg.nms)
        if max_proposals > 0:
