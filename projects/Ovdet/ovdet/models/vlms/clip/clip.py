...
__all__ = [
    'available_models', 'load', 'tokenize', 'tokenize_dynamic',
    'get_only_word_tokens'
]
...
def load(name: str,
    ...
    Parameters ---------- name : str A model name listed by
    `clip.available_models()`, or the path to a model checkpoint containing
    the state_dict
    ...
    preprocess : Callable[[PIL.Image], torch.Tensor] A torchvision transform
    that converts a PIL image into a tensor that the returned model can take
    as its input
    """
    if name in _MODELS:
    ...
    try:
        ...
        if jit:
            warnings.warn(
                f'File {model_path} is not a JIT archive. Loading as a state '
                f'dict instead ')
            jit = False
...
def tokenize(texts: Union[str, List[str]],
    ...
    truncate: bool Whether to truncate the text in case its encoding is
    longer than the context length

    Returns ------- A two-dimensional tensor containing the resulting
    tokens, shape = [number of input strings, context_length]
    """
