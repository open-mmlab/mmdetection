...
class RandomSamplingNegPosV2(BaseTransform):
    ...
    def rec_aug(self, results):
        ...
        if results['dataset_mode'] == 'REC':
            ...
            if num_negatives != -1:
                if num_negatives > len(keys):
                    num_negatives = len(keys)

                for i in np.random.choice(
                        keys, size=num_negatives, replace=False):
                    if i not in results['img_path']:
                        others_exp = results['image_to_exp'][i]
                        if isinstance(others_exp, list):
                            others_exp = random.choice(others_exp)
                        if isinstance(others_exp, list):
                            others_exp = random.choice(others_exp)
                        negative_label_list.add(others_exp)
