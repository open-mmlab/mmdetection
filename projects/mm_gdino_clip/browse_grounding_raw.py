...
# /home/PJLAB/huanghaian/dataset/coco2014/ mdetr_annotations/finetune_refcocog_train_ref.json train2014 --not-show --shuffle -o rex
def parse_args():
...
def main():
    ...
    for i in dataset_index[:args.show_num]:
        ...
        if 'detection' in item:
            ...
            referring = item['referring']['instances']
            ...
            for i, p in enumerate(phrases):
                ...
                for _p in p:
                    chunk_p = [
                        _p[i:i + chunk_size] for i in range(0, len(_p), chunk_size)
                    ]
                    for cp in chunk_p:
                        visualizer.draw_texts(
                            cp,
