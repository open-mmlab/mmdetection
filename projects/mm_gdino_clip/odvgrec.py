...
class ODVGRECDataset(BaseDetDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for data in data_list:
            ...
            if 'referring' in data:
                self.dataset_mode = 'REC'
            ...
            elif self.dataset_mode == 'REC':
                anno = data.get('referring', {})
                instances = [obj for obj in anno.get('instances', [])]
                bboxes = [obj['bbox'] for obj in instances]
                bbox_exp = [obj['exp'] for obj in instances]

                self.image_to_exp[img_path] = bbox_exp

                bbox_labels = list(range(len(bboxes)))

                phrases = {}
                instances = []
                i = 0
                for bbox, exp, label in zip(bboxes, bbox_exp, bbox_labels):
                    instance = {}
                    x1, y1, x2, y2 = bbox
                    inter_w = max(0, min(x2, data['width']) - max(x1, 0))
                    inter_h = max(0, min(y2, data['height']) - max(y1, 0))
    ...
    def prepare_data(self, idx: int):
        ...
        if self.test_mode is False:
            ...
            if self.dataset_mode == 'REC':
                data_info['image_to_exp'] = self.image_to_exp
