...
def write_csv(datas, args):
    ...
    for dataset in sorted(os.listdir(datas)):
        ...
        if num==1:  # generate headers
            result_csv = os.path.join(args.work_dirs,f'{latest_log_name}_eval.csv')
            print(result_csv)
            ...
            with open(result_csv, mode='a') as f:
                writer = csv.writer(f)
                header2 = ['Dataset', 'Category', 'train', 'valid', 'mAP', 'mAP50', 'mAP_s', 'mAP_m', 'mAP_l']
...
def wb_align(file, pair_ls):
    ...
    for pair in pair_ls:
        ...
        ws[f'{pair[0]}'].alignment = Alignment(horizontal='center', vertical='center')
    wb.save(file)
...
def sum_excel(in_csv, out_xlsx):
    ...
    for key, value in average:
        ...
        for i in range(1,8):
            if i==1 or i==2:
                ...
                df_cate.append(format(value.iloc[:, i].astype(float).replace('', np.nan).mean(), '.4f'))
    ...
    for i in range(1,8):
        if i==1 or i==2:
            ...
            df_cate.append(format(df_new.iloc[1:, i].astype(float).replace('', np.nan).mean(), '.4f'))
