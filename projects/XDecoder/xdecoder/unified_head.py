...
class XDecoderUnifiedhead(nn.Module):
    ...
    def pre_process(self, batch_data_samples, device):
        ...
        if self.task != 'caption':
            ...
            if self.task in ['semseg', 'instance', 'panoptic']:
                self.predictor.lang_encoder.get_text_embeddings(all_text_prompts + ["background"])
            elif self.task == 'ref-semseg':
                token_info = self.predictor.lang_encoder.get_text_token_embeddings(all_text_prompts, norm=False)
                ...
                token_info = self.predictor.lang_encoder.get_text_token_embeddings(all_text_prompts, norm=True)
                extra['class_emb'] = token_info['class_emb']
    ...
    def predict(self, features, batch_data_samples):
        ...
        # pre process
        extra, all_text_prompts, stuff_text_prompts = self.pre_process(batch_data_samples, mask_features.device)

        # transformer decoder forward
        predictions = self.predictor(multi_scale_features, mask_features, extra=extra)

        # post process
        return self.post_process(predictions, batch_data_samples, all_text_prompts, stuff_text_prompts)

    def post_process(self, predictions, batch_data_samples, all_text_prompts, stuff_text_prompts):
        batch_img_metas = [
        ...
        if self.task == 'caption':
            for text, data_samples in zip(predictions['pred_caption'], batch_data_samples):
            ...
            if 'pred_sem_seg' in batch_data_samples[0]:
                for img_metas, data_samples in zip(batch_img_metas, batch_data_samples):
                    ...
                    pred_sem_seg = retry_if_cuda_oom(self._semantic_inference)(None, mask_pred_result,
                                                                              text_prompts)
            ...
            for mask_cls_result, mask_pred_result, img_metas, data_samples in zip(mask_cls_results,
                ...
                if self.task == 'semseg':
                    pred_sem_seg = retry_if_cuda_oom(self._semantic_inference)(mask_cls_result, mask_pred_result,
                                                                              all_text_prompts)
                    data_samples.pred_sem_seg = pred_sem_seg
                elif self.task == 'instance':
                    pred_instances = retry_if_cuda_oom(self._instance_inference)(mask_cls_result, mask_pred_result,
                                                                                all_text_prompts)
                    data_samples.pred_instances = pred_instances
                elif self.task == 'panoptic':
                    # TODO: support batch
                    stuff_text_prompts = stuff_text_prompts[0]
                    thing_text_prompts = [x for x in all_text_prompts if x not in stuff_text_prompts]
                    pred_panoptic_seg = retry_if_cuda_oom(self._panoptic_inference)(mask_cls_result,
                                                                                   mask_pred_result,
                                                                                   thing_text_prompts,
                                                                                   stuff_text_prompts)
            ...
            for mask_pred_result, img_metas, data_samples in zip(
                ...
                pred_sem_seg = retry_if_cuda_oom(self._semantic_inference)(None, mask_pred_result, all_text_prompts)
                data_samples.pred_sem_seg = pred_sem_seg
        elif self.task == 'retrieval':
