...
def parse_args():
    parser = argparse.ArgumentParser(description='Convert NuScenes MMDetection3D format to MMDetection format')
    parser.add_argument('--data-root', default='data/nuscenes', help='NuScenes data root')
    parser.add_argument('--out-dir', default='data/nuscenes/annotations', help='Output directory')
    parser.add_argument('--camera', default='CAM_FRONT', help='Camera to use for 2D detection')
    parser.add_argument('--pkl-prefix', default='nuscenes_infos', help='Prefix of the pkl files')
...
def main():
    ...
    for split in ['train', 'val']:
        ...
        for info in tqdm(data_list, desc=f"Processing {split} samples"):
            ...
            if not osp.isabs(img_path):
                ...
                if not osp.exists(full_path):
                    ...
                    cam_dir = args.camera
                    alternate_path = osp.join(args.data_root, 'samples', cam_dir, filename)
                    if osp.exists(alternate_path):
                        full_path = alternate_path
                    else:
                        # Skip if we can't find the image
                        print(f"Warning: Could not find image at {full_path} or {alternate_path}, skipping")
                        continue
                img_path = full_path
        ...
        print(f"Created {split} dataset with {len(coco_data['images'])} images and {len(coco_data['annotations'])} annotations.")
