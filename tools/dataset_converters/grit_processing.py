...
def count_download_image(download_json_dir, logger):
    parquet_files = [f for f in os.listdir(download_json_dir) if f.endswith('.json')]
    len = 0
    ...
    for file in parquet_files:
        ...
        logger.info(file + 'has ' + str(data['successes']) + ' successful images')

    logger.info('all files finished.', str(len), 'images have been successfully downloaded.')
...
def main(args):
    ...
    if args.image_dir != None:
        all_file_name = [os.path.join(args.image_dir, file) for file in os.listdir(args.image_dir) if file.endswith('.tar')]
        all_file_name.sort()
