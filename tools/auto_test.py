...
for model_family in os.listdir(args.model_path):
    ...
    for model in os.listdir(model_family_dir):
        ...
        day_time = log[0].split('_')[0]
        src = os.path.join(model_dir, log[0])
        log = os.path.join(model_dir, model + '_' + log[0])
        os.rename(src, log)
        ...
        # rename cpt
        try:
            hash = cpt[0].split('-')[-1].split('.')[0]
        except Exception:
            assert len(hash) == 8, 'please push model: {}'.format(model)
            exit()

        new_cpt_name = model + '_' + day_time + '-' + hash + '.pth'
        print('rename {} to {}'.format(cpt[0], new_cpt_name))
        src = os.path.join(model_dir, cpt[0])
        cpt = os.path.join(model_dir, new_cpt_name)
        os.rename(src, cpt)
