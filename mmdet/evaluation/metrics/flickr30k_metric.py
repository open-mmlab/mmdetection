...
class Flickr30kMetric(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for pred, gt_boxes, gt_labels, phrases in zip(pred_list, gt_list, gt_label_list, phrase_list):
            pred_boxes = pred['bboxes'].cpu().numpy()
            pred_labels = pred['labels'].cpu().numpy()
            for i, phrase in enumerate(phrases):
                cur_index = pred_labels==i
                cur_boxes = pred_boxes[cur_index]
                tar_index = [index for index, value in enumerate(gt_labels) if value == i]
                tar_boxes = gt_boxes[tar_index]
                if self.merge:
                    tar_boxes = self.merge_boxes(tar_boxes)
                if len(cur_boxes) == 0:
                    cur_boxes = [[0., 0., 0., 0.]]
                ious = bbox_overlaps(np.asarray(cur_boxes), np.asarray(tar_boxes))
                for k in self.topk:
                    maxi = 0
                    if k == -1:
                        maxi = ious.max()
