...
Tensor affine_grid_generator_4D_forward(
    const Tensor& theta,
    int64_t N,
    int64_t C,
    int64_t H,
...
Tensor affine_grid_generator_5D_forward(
    const Tensor& theta,
    int64_t N,
    int64_t C,
    int64_t D,
...
Tensor affine_grid_generator_forward(const Tensor& theta, IntArrayRef size, bool align_corners) {
  if (size.size() == 4) {
    return affine_grid_generator_4D_forward(
        theta, size[0], size[1], size[2], size[3], align_corners);
  } else {
    return affine_grid_generator_5D_forward(
