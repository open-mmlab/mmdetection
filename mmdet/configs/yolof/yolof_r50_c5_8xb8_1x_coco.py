...
from mmdet.models.dense_heads.yolof_head import YOLOFHead
...
from mmdet.models.task_modules.assigners.uniform_assigner import UniformAssigner
...
from mmdet.datasets.transforms.transforms import Resize, RandomFlip, RandomShift, Resize
...
optim_wrapper.merge(
    dict(
        ...
        paramwise_cfg=dict(
            norm_decay_mult=0., custom_keys={'backbone':
                                             dict(lr_mult=1. / 3)})))
...
train_dataloader.merge(
    dict(batch_size=8, num_workers=8, dataset=dict(pipeline=train_pipeline)))
