...
model.merge(dict(roi_head=dict(bbox_head=dict(num_classes=1))))
metainfo = {
    'classes': ('person', ),
    'palette': [
        (220, 20, 60),
...
train_dataloader.merge(dict(dataset=dict(metainfo=metainfo)))
val_dataloader.merge(dict(dataset=dict(metainfo=metainfo)))
test_dataloader.merge(dict(dataset=dict(metainfo=metainfo)))
