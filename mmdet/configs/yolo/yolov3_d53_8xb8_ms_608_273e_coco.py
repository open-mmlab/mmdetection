...
from mmdet.models.losses.cross_entropy_loss import CrossEntropyLoss, CrossEntropyLoss, CrossEntropyLoss
from mmdet.models.losses.mse_loss import MSELoss
from mmdet.models.task_modules.assigners.grid_assigner import GridAssigner
...
from mmdet.datasets.transforms.transforms import Expand, MinIoURandomCrop, RandomFlip, PhotoMetricDistortion, Resize
...
model = dict(
    ...
    train_cfg=dict(
        assigner=dict(
            type=GridAssigner, pos_iou_thr=0.5, neg_iou_thr=0.5,
            min_pos_iou=0)),
    test_cfg=dict(
        nms_pre=1000,
        min_bbox_size=0,
...
train_cfg.merge(dict(max_epochs=273, val_interval=7))
...
default_hooks.merge(dict(checkpoint=dict(type=CheckpointHook, interval=7)))
