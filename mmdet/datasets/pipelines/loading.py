...
class LoadImagesFromFile(object):
    ...
    def __call__(self, results):
        ...
        Different from `LoadImagesFromFile`, most fields are lists in
        `results`.
        ...
        for i in range(len(results["img_info"])):
            if results['img_prefix'] is not None:
                filename = osp.join(results['img_prefix'],
                                    results['img_info'][i]['filename'])
            else:
                filename = result['img_info'][i]['filename']
            ...
            ori_filenames.append(results['img_info'][i]['filename'])
            imgs.append(img)
            img_shapes.append(img.shape)
            ori_shapes.append(img.shape)

        results['filename'] = filenames
        results['ori_filename'] = ori_filenames
        results['img'] = imgs
...
# author: Mingtao
@PIPELINES.register_module()
class LoadMultiImagesAnnotations(object):
    """Load bbox annotations for multiple images.
    ...
    def _load_bboxes(self, results):
        ...
        ann_infos = results['ann_info']
        results['gt_bboxes'] = [ann_info['bboxes'].copy() for ann_info in ann_infos]
        results['gt_bboxes_ignore'] = []

        gt_bboxes_ignores = [ann_info.get('bboxes_ignore', None) for ann_info in ann_infos]
    ...
    def _load_labels(self, results):
        ...
        results['gt_labels'] = [ann_info['labels'].copy() for ann_info in results['ann_info']]
        return results
