...
class InstaBoost(object):
    """
    Data augmentation method in paper "InstaBoost: Boosting Instance Segmentation Via Probability Map Guided Copy-Pasting"
    ...
    def __init__(self, 
                ...
                hflag=False, 
                aug_ratio=0.5):
        self.ibcfg = instaboost.InstaBoostConfig(action_candidate, action_prob, scale, dx, dy, theta, color_prob, hflag)
    ...
    def __call__(self, results):
        ...
        if np.random.choice([0,1],p=[1-self.aug_ratio,self.aug_ratio]):
            anns, img = instaboost.get_new_data(anns, img, self.ibcfg, background=None)
        results = self._parse_anns(results, anns, img)
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += ('(ibcfg={}, aug_ratio={})').format(self.ibcfg, self.aug_ratio)
        return repr_str