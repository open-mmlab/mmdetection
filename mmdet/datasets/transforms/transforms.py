...
class RandomErasing(BaseTransform):
    ...
    Args:
        n_patches (int or tuple[int, int]): Number of regions to be dropped.
            If it is given as a list, number of patches will be randomly
            selected from the closed interval [``n_patches[0]``,
            ``n_patches[1]``].
        ratio (float or tuple[float, float]): The ratio of erased regions.
            It can be ``tuple[float, float]`` to use a fixed ratio or
            ``tuple[float, float]`` to randomly choose ratio from the interval.
    ...
    def _transform_bboxes(self, results: dict, patches: List[list]) -> None:
        ...
        for patch in patches:
            px1, py1, px2, py2 = patch
            bboxes = results['gt_bboxes']
            valid_inds = ~((bboxes[:, 0] > px1) & (bboxes[:, 1] > py1) &
                           (bboxes[:, 2] < px2) & (bboxes[:, 3] < py2))
            results['gt_bboxes'] = bboxes[valid_inds]
            results['gt_bboxes_labels'] = results['gt_bboxes_labels'][
                valid_inds]
