...
class AnchorGenerator_Ultra(object):
    ...
    def __gen_base_anchors(self,ctr):
    	x_ctr, y_ctr = ctr
    	base_anchors = list()
    	for min_box in self.min_boxes:
    		w = min_box
    ...
    def _grid_anchors(self, featmap_size, ctr, device='cuda'):
        ...
        __base_anchors = self.__gen_base_anchors(ctr).to(device)
        feat_w = featmap_size[1]
        feat_h = featmap_size[0] 
        shift_x = torch.arange(0, feat_w, device=device) * (self.input_size[0]/feat_w)
        shift_y = torch.arange(0, feat_h, device=device) * (self.input_size[1]/feat_h)
