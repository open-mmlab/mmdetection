...
class YOLOAnchorGenerator(AnchorGenerator):
    ...
    def responsible_flags(self, featmap_sizes, gt_bboxes, device='cuda'):
		...
		Args:
		    featmap_sizes (list(tuple)): List of feature map sizes in
		        multiple feature levels.
		    gt_bboxes (Tensor): Ground truth boxes, shape (nx4) in (xyxy) format
		    device (str): Device where the anchors will be put on.

		Return:
		    list(torch.Tensor): responsible flags of anchors in multiple levels.
		"""
		assert self.num_levels == len(featmap_sizes)
		...
		for i in range(self.num_levels):
		    anchor_stride = self.strides[i]
		    feat_h, feat_w = featmap_sizes[i]
		    flags = self.single_level_responsible_flags((feat_h, feat_w),
		                                          		gt_bboxes_cx, gt_bboxes_cy,
		                                          		anchor_stride, 
		                                          		self.num_base_anchors[i],
		                                          		device=device)
    ...
    def single_level_responsible_flags(self,
        ...
        Args:
            featmap_size (tuple[int]): The size of feature maps.
            gt_bboxes_cx, gt_bboxes_cy (tensor, shape(n, )): center of gt_bboxes
