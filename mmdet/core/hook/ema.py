...
class BaseEMAHook(Hook):
    ...
    process. All parameters have an ema backup. EMAHook takes priority
    over EvalHook and CheckpointHook.

    Note: The original model parameters are actually saved in ema field
    after train.

    Args:
        skip_buffer (bool): Whether to skip buffer, it does not perform the
           ema operation. Default to False.
        interval (int): Update ema parameter every interval iteration.
            Defaults to 1.
        resume_from (str, Optional): The checkpoint path. Defaults to None.
    """

    def __init__(self, skip_buffer=False, interval=1, resume_from=None):
...
class LinearDecayEMAHook(BaseEMAHook):
    ...
    def __init__(self, momentum=0.0002, warm_up=100, **kwargs):
        ...
        self.decay_fun = lambda x: 1 - min(momentum**self.interval, (1 + x) /
                                           (warm_up + x))
