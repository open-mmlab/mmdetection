...
@IOU_CALCULATORS.register_module()
class BboxGIoU2D(object):
    """2D GIoU Calculator."""

    def __call__(self, bboxes1, bboxes2, is_aligned=False, eps=1e-6):
        """Calculate GIoU between 2D bboxes.
        ...
        if bboxes2.size(-1) == 5:
            bboxes2 = bboxes2[..., :4]
        return bbox_gious(bboxes1, bboxes2, is_aligned, eps)

    def __repr__(self):
        """str: a string describing the module"""
        repr_str = self.__class__.__name__
        repr_str += f'(is_aligned={self.is_aligned}, '
        repr_str += f'eps={self.eps})'
        return repr_str


def bbox_gious(bboxes1, bboxes2, is_aligned=False, eps=1e-6):
    """Calculate gious between two set of bboxes.

    If `is_aligned` is ``False``, then calculate the gious between each bbox
    of bboxes1 and bboxes2, otherwise the gious between each aligned pair of
    bboxes1 and bboxes2.
