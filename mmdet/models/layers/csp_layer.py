...
class C2fLayer(BaseModule):
    ...
    def __init__(self,
        ...
        if channel_attention:
            self.attention = ChannelAttention(2 * self.hidden_channels)

    def forward(self, x: Tensor) -> Tensor:
        x = list(self.main_conv(x).split((self.hidden_channels, self.hidden_channels), dim=1))
        x.extend(block(x[-1]) for block in self.blocks)
