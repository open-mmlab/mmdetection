...
class GLIP(SingleStageDetector):
    ...
    def get_tokens_positive_and_prompts(self, original_caption, custom_entities: bool = False):
    ...
    def predict(self,
        ...
        if 'custom_entities' in batch_data_samples[0]:
            # Assuming that the `custom_entities` flag inside a batch is always the same
        ...
        if text_prompts != self._text_prompts:
            ...
            if len(set(text_prompts)) == 1:
                ...
                _positive_maps_and_prompts = [self.get_tokens_positive_and_prompts(text_prompts[0],
                                                                                   custom_entities)] * len(
                    batch_inputs)
            else:
                _positive_maps_and_prompts = [self.get_tokens_positive_and_prompts(text_prompt, custom_entities) for
                                              text_prompt in
                                              text_prompts]
