...
class DETR(SingleStageDetector):
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        # NOTE currently only the feature from backbone's last stage is
        # supported and used.
        x = x[-1]

        # construct binary mask which used for transformer.
        # NOTE following the official DETR repo, non-zero values representing
        # ignored positions, while zero values means valid positions.
        pad_h, pad_w, _ = img_metas[0]['pad_shape']
        img_h, img_w, _ = img_metas[0]['img_shape']
        masks = torch.ones((batch_size, pad_h, pad_w)).to(x.device)
