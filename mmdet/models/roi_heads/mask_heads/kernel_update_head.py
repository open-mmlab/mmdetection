...
class KernelUpdateHead(BaseModule):
    ...
    def __init__(
        ...
        if loss_rank is not None:
            self.loss_rank = MODELS.build(loss_rank)
        else:
    ...
    def _init_layers(self):
        self.attention = MODELS.build(self.attn_cfg)
        self.attention_norm = build_norm_layer(
            self.attn_ffn_norm_cfg,
            self.in_channels * self.conv_kernel_size**2)[1]

        self.kernel_update_conv = MODELS.build(self.kernel_updator_cfg)
        ...
        self.ffn = MODELS.build(self.ffn_cfg)
