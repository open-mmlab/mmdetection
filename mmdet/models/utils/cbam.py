...
class ChannelGate(nn.Module):
    ...
    def forward(self, x):
        ...
        for pool_type in self.pool_types:
            if pool_type=='avg':
                avg_pool = F.avg_pool2d( x, kernel_size=[height, width], stride=[height, width])
                channel_att_raw = self.mlp( avg_pool )
            elif pool_type=='max':
                max_pool = F.max_pool2d( x, kernel_size=[height, width], stride=[height, width])
                channel_att_raw = self.mlp( max_pool )
            elif pool_type=='lp':
                lp_pool = F.lp_pool2d( x, 2, [height, width], stride=[height, width])
