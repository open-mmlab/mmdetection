...
def multiclass_nms(multi_bboxes,
    ...
    points = multi_points[:, None].expand(multi_scores.size(0), num_classes, 2)
    strides = multi_strides[:, None].expand(multi_scores.size(0), num_classes)
    ...
    bboxes, scores, labels, kernels, points, strides = \
        bboxes[inds], scores[inds], labels[inds], kernels[inds],\
        points[inds], strides[inds]
    if inds.numel() == 0:
...
def parse_dynamic_params(params, channels, weight_nums, bias_nums):
    ...
    for index in range(num_layers):
        if index < num_layers - 1:
            ...
            weight_splits[index] = weight_splits[index].reshape(
                num_instances * channels, -1, 1, 1)
            bias_splits[index] = bias_splits[index].reshape(num_instances *
                                                            channels)
        else:
            # out_channels x in_channels x 1 x 1
            weight_splits[index] = weight_splits[index].reshape(
...
class CondInstHead(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        self.controller = nn.Conv2d(self.feat_channels, 169, 3, padding=1)
        ...
        tower.append(nn.Conv2d(128, 8, kernel_size=1, stride=1))
    ...
    def init_weights(self):
        ...
        for m in self.mask_refine.modules():
    ...
    def loss(
        ...
        cls_reg_targets = self.get_targets(points_list, gt_bboxes, gt_labels,
                                           gt_masks_list)
        ...
        if len(pos_inds):
            ...
            pos_decode_bbox_pred = distance2bbox(pos_points, pos_bbox_pred)
            pos_decode_bbox_targets = distance2bbox(pos_points,
                                                    pos_bbox_targets)

            # centerness weighted iou loss
        else:
            pos_centerness_targets = flatten_cls_scores.new_zeros(1)
        ...
        flatten_kernel_preds = [
            kernel.permute(0, 2, 3, 1).reshape(num_imgs, -1, 169)
            for kernel in kernel_preds
        ]
        ...
        for i in range(num_imgs):
            ...
            pos_inds = ((flatten_labels_i >= 0)
                        & (flatten_labels_i < self.num_classes)).nonzero(
                            as_tuple=False).squeeze(1)
            if len(pos_inds) == 0:
                loss_mask = loss_mask + (flatten_kernel_preds.sum() +
                                         mask_feats.sum()) * 0
                continue
            # mask feat
            ...
            mask_head_inputs = self.relative_coordinate_feature_generator(
                mask_feat, points, strides)
            weights, biases = parse_dynamic_params(mask_head_params, 8,
                                                   self.weight_nums,
                                                   self.bias_nums)
        ...
        loss_mask = loss_mask / max(num_mask, 1)
    ...
    def relative_coordinate_feature_generator(self, mask_feat,
        ...
        locations = compute_locations(H, W, stride=8, device=mask_feat.device)
        relative_coordinates = instance_locations.reshape(
        ...
        relative_coordinates = relative_coordinates / (
            strides.float().reshape(-1, 1, 1) * 8.0)
        ...
        coordinates_feat = torch.cat([
            ...
            mask_feat.repeat(num_instance, 1, 1, 1)
        ],
                                     dim=1)
    ...
    def get_bboxes(self,
        ...
        mlvl_points, mlvl_strides = self.get_points(featmap_sizes,
                                                    bbox_preds[0].dtype,
                                                    bbox_preds[0].device)
        ...
        for img_id in range(len(img_metas)):
            ...
            det_bboxes, det_labels, det_masks = self._get_bboxes_single(
                ...
                kernel_pred_list, mask_feats_i, mlvl_points, mlvl_strides,
                img_shape, scale_factor, ori_shape, cfg, rescale, with_nms)
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, centerness, kernel_pred, \
            points, strides in zip(
                cls_scores, bbox_preds, centernesses, kernel_preds,
        ...
        det_bboxes, det_labels, det_kernels_pred, \
            det_points, det_strides = multiclass_nms(
                mlvl_bboxes,
                mlvl_scores,
                mlvl_kernels_pred,
                flatten_mlvl_points,
        ...
        if det_bboxes.shape[0] > 0:
            ...
            mask_head_inputs = self.relative_coordinate_feature_generator(
                mask_feat, det_points, det_strides)
            weights, biases = parse_dynamic_params(mask_head_params, 8,
                                                   self.weight_nums,
                                                   self.bias_nums)
            mask_logits = self.mask_heads_forward(mask_head_inputs, weights,
    ...
    def get_targets_single(self, gt_bboxes, gt_labels, gt_masks, points,
        ...
        if self.center_sampling:
            ...
            masss_gt_masks = gt_masks.sum((1, 2))
            _, h, w = gt_masks.shape
            grid_h = torch.arange(h, device=gt_masks.device)[:, None]
            grid_w = torch.arange(w, device=gt_masks.device)
            center_ys = (gt_masks * grid_h).sum(
                (1, 2)) / masss_gt_masks.clamp(min=1)
            center_xs = (gt_masks * grid_w).sum(
                (1, 2)) / masss_gt_masks.clamp(min=1)

            center_xs = center_xs[None, :]
            center_ys = center_ys[None, :]
            # bbox_center_xs = (gt_bboxes[0,:, 0] + gt_bboxes[0,:, 2]) / 2
            # bbox_center_ys = (gt_bboxes[0,:, 1] + gt_bboxes[0,:, 3]) / 2
            center_gts = torch.zeros_like(gt_bboxes)
            # num_points * num_gt
            stride = bbox_targets.new_zeros(bbox_targets.shape[:2])
    ...
    def get_points(self, featmap_sizes, dtype, device):
        ...
        for i in range(len(featmap_sizes)):
            points, strides = self.get_points_single(featmap_sizes[i],
