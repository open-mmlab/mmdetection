...
class YOLOV3Head(BaseDenseHead):
    ...
    def _preprocess_target_single_img(self,
        ...
        for gt_bbox, gt_label in zip(gt_bboxes, gt_labels):
            ...
            # in mmdet 1.x, raw label starts from 1,
            # need to minus 1 to compensate that
            # However, in mmdet 2.x, label “K” means background, and labels
            # [0, K-1] correspond to the K = num_categories object categories.
            gt_label_one_hot = F.one_hot(
                gt_label, num_classes=self.num_classes_no_bkg).float()
            # TODO: Check is pending

            gt_label_one_hot = gt_label_one_hot * (
                1 -
                one_hot_smoother) + one_hot_smoother / self.num_classes_no_bkg
