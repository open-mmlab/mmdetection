...
def postprocess(prediction, num_classes, score_thr=0.7, nms_thr=0.45):
    ...
    output = [(prediction.new_zeros(
        (0, 5)), prediction.new_zeros((0, ), dtype=torch.long))
    ...
    for i, image_pred in enumerate(prediction):
        ...
        nms_out_index = torchvision.ops.batched_nms(dets[:, :4], dets[:, 4],
                                                    labels, nms_thr)
        dets = dets[nms_out_index]
        labels = labels[nms_out_index]
        output[i] = (dets, labels)
