...
from mmcv.cnn import (Conv2d, Linear, bias_init_with_prob, constant_init,
                      uniform_init)
from mmcv.runner import ModuleList

from mmdet.models.utils.transformer import MLP, inverse_sigmoid
...
class DABDETRHead(DETRHead):
    ...
    def _init_layers(self):
        ...
        self.bbox_embed_diff_each_layer = \
            self.transformer.bbox_embed_diff_each_layer
        self.nb_dec = self.transformer.nb_dec
        self.input_proj = Conv2d(
            self.in_channels, self.embed_dims, kernel_size=1)
        self.fc_cls = Linear(self.embed_dims, self.cls_out_channels)
        if self.bbox_embed_diff_each_layer:
            self.fc_reg = ModuleList([
    ...
    def forward_single(self, x, img_metas):
        ...
        if not self.bbox_embed_diff_each_layer:
            # inverse_sigmoid -- to match official repo, still slight different
