...
def SampleFreeHead(head, model_key, init_prior_file, loss_cls_scale, **kwargs):
    class SFHead(eval(head)):
        def __init__(self, **kwargs):
            ...
            self.sampling = False
            #when use down-sample, the avg_factor of bbox loss in rpn stage is num_total_pos + num_total_neg,
            #however if we use sample-free, the avg_factor is num_total_pos, then the bbox loss will be larger,
            #so we need to rescale the loss, here we set loss_bbox_avg_factor = 8, the value refer to the Official Repo,
        ...
        def init_weights(self):
            super().init_weights()
            conv_cls_attr = [attr for attr in dir(self) 
                             if 'cls' in attr and isinstance(getattr(self, attr), nn.Conv2d)]
            assert len(conv_cls_attr) == 1
            bias_cls = bias_init_with_prob(self.prior)
            normal_init(getattr(self, conv_cls_attr[0]), std=0.01, bias=bias_cls)


        def loss(self, *args, **kwargs):
            loss_dict = super().loss(*args, **kwargs)
            loss_cls_key = [key for key in loss_dict.keys() if 'cls' in key][0]
            loss_bbox_key = [key for key in loss_dict.keys() if 'bbox' in key][0]
            loss_cls = loss_dict[loss_cls_key]
            loss_bbox = loss_dict[loss_bbox_key]
            loss_bbox = list(map(lambda x: x / self.loss_bbox_avg_factor, loss_bbox))
...
#refer to https://github.com/ChenJoya/sampling-free/blob/master/maskrcnn_benchmark/modeling/sampling_free.py
...
def load_prior(model_key, filename):
    if not os.path.exists(filename):
        raise FileNotFoundError(f'{filename} is not existed. please calculating prior before training.')
    ...
    with open(filename, 'r') as f:
        ...
        if model_key in model_prior_dict:
            ...
            raise KeyError(f'{model_key} is not existed. please calculating it before training.')
