...
class UltraSlim(nn.Module):
	...
	def __init__(self,input_size):
		...
		self.extras = ModuleList([
			Sequential(
				Conv2d(in_channels=self.base_net.base_channel * 16, out_channels=self.base_net.base_channel * 4, kernel_size=1),
				ReLU(),
				SeperableConv2d(in_channels=self.base_net.base_channel * 4, out_channels=self.base_net.base_channel * 16, kernel_size=3, stride=2, padding=1),
				ReLU()
			)
		])
	...
	def forward(self, x):
		...
		for end_layer_index in self.source_layer_indexes:
			for layer in self.base_net.model[start_layer_index: end_layer_index]:
				x = layer(x)
			y = x
