...
class IPUDarknet(Darknet):
    """Darknet backbone(IPU version), has one more argument named serial_num.
    This backbone will divide the data into {serial_num} batches and calculate
    them one by one in training mode.

    Args:
        serial_num (int): Input a batch of data, it will divide the data into
            {serial_num} batches and calculate them in turn. This method is
            mainly used to reduce the peak memory.
        kwargs (dict): See Darknet for more details.
    """

    def __init__(self, serial_num, **kwargs):
