...
class MaskFormerHead(AnchorFreeHead):
    ...
    Args:
        ...
        transformer_decoder (obj:`mmcv.ConfigDict`|dict): Config for transformer
            decoder. Defaults to None.
        positional_encoding (obj:`mmcv.ConfigDict`|dict): Config for transformer
            decoder position encoding. Defaults to None.
        loss_cls (obj:`mmcv.ConfigDict`|dict): Config of the classification loss.
            Defaults to `CrossEntropyLoss`.
        loss_mask (obj:`mmcv.ConfigDict`|dict): Config of the mask loss. Defaults 
            to `FocalLoss`. 
        loss_dice (obj:`mmcv.ConfigDict`|dict): Config of the dice loss. Defaults 
            to `DiceLoss`.
        train_cfg (obj:`mmcv.ConfigDict`|dict): Training config of Maskformer head.
        test_cfg (obj:`mmcv.ConfigDict`|dict): Testing config of Maskformer head.
    ...
    def __init__(self,
        ...
        self.query_embed = nn.Embedding(self.num_queries, mask_dim)
        
        self.cls_embed = nn.Linear(conv_dim, self.num_classes + 1)
        self.mask_embed = nn.Sequential(
            nn.Linear(conv_dim, conv_dim),
            nn.ReLU(inplace=True),
            nn.Linear(conv_dim, conv_dim),
            nn.ReLU(inplace=True),
            nn.Linear(conv_dim, mask_dim))
        ...
        if class_weight is not None and (self.__class__ is MaskFormerHead):
            assert isinstance(class_weight, float), 'Expected ' \
                ...
                f'{type(class_weight)}.'
            # NOTE following the official MaskFormerHead rep0, bg_cls_weight means
            # relative classification weight of the VOID class.
    ...
    def _preprocess_ground_truth_single(self, 
                                  gt_labels, 
                                  gt_masks, 
                                  gt_semantic_seg):
        ...
        semantic_labels = torch.unique(gt_semantic_seg, 
                                    sorted=False, 
    ...
    def get_targets(self, 
                ...
                gt_masks_list, 
                img_metas):
        """Compute classification and mask targets for all images for 
            a decoder layer.
        ...
        Returns:
            tuple[list[Tensor]]: a tuple containing the following targets.
                ...
                - label_weights_list (list[Tensor]): Label weights of all images.
                    Each with shape [num_query, ].
        ...
        (labels_list, label_weights_list, mask_targets_list, 
         mask_weights_list, pos_inds_list, neg_inds_list) = multi_apply(
             self._get_target_single, cls_scores_list, mask_preds_list, 
    ...
    def _get_target_single(self, 
        ...
        mask_weights = torch.zeros((self.num_queries, ), 
                                    dtype=torch.float32, device=mask_pred.device)
    ...
    def loss(self, 
        ...
        num_dec_layer = 0
        for loss_cls_i, loss_mask_i, loss_dice_i in zip(losses_cls[:-1],
                                                       losses_mask[:-1],
                                                       losses_dice[:-1]):
    ...
    def loss_single(self, 
        ...
        Args:
            ...
            gt_masks_list (list[Tensor]): Ground truth mask for each image, each
                with shape (n, h, w).
            img_metas (list[dict]): List of image meta information.
        ...
        (labels_list, label_weights_list, mask_targets_list, 
         mask_weights_list, num_total_pos, num_total_neg) = self.get_targets(
                cls_scores_list, mask_preds_list, gt_labels_list, 
                gt_masks_list, img_metas)
        
        labels = torch.stack(labels_list, dim=0) # shape [bs, bq]
        label_weights = torch.stack(label_weights_list, dim=0) # shape [bs, bq]
        ...
        if mask_targets.shape[0] == 0:
            # zero match
            loss_dice = torch.tensor(0.0, dtype=torch.float32, device=mask_preds.device)
            loss_mask = torch.tensor(0.0, dtype=torch.float32, device=mask_preds.device)
            return loss_cls, loss_mask, loss_dice

        num_total_masks = reduce_mean(
            cls_scores.new_tensor([num_total_pos]))
        ...
        mask_preds = mask_preds.reshape(-1, 1) # [n_gts, h, w] -> [n_gts * h * w, 1]
        mask_targets = mask_targets.reshape(-1) # [n_gts, h, w] -> [n_gts * h * w]
        #! 1 - mask_targets
        loss_mask = self.loss_mask(mask_preds, 1 - mask_targets,
    ...
    def forward(self, feats):
        ...
        query_embed = query_embed.unsqueeze(1).repeat(1, bs, 1) # shape = [nq, bs, em]
    ...
    def forward_train(self, 
        ...
        """
        assert gt_bboxes_ignore == None # not consider ignoring bboxes

        # forward 
        all_cls_scores, all_mask_preds = self(feats)     

        # preprocess ground truth
        gt_labels, gt_masks = self.preprocess_ground_truth(gt_labels, 
            gt_masks, gt_semantic_seg)

        # loss
        losses = self.loss(all_cls_scores, all_mask_preds, 
                           gt_labels, gt_masks, 
                           img_metas)
    ...
    def simple_test(self, feats, img_metas, rescale=False):
        ...
        Returns:
            list[dict[str, np.array]]: semantic segmentation results and panoptic
        ...
        for mask_cls_result, mask_pred_result, meta in zip(
            ...
            if rescale:
                # return result in original resulotion
            ...
            result = {
                "pan_results": mask.detach().cpu().numpy()
            }
    ...
    def post_process(self, mask_cls, mask_pred):
        ...
        panoptic_seg = torch.zeros((h, w), dtype=torch.int32,
            device=cur_masks.device).fill_(self.num_classes)
