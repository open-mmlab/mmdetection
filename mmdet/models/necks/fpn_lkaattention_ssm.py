...
class lka_FPN_ssm(BaseModule):
    ...
    def __init__(self,
        ...
        super(lka_FPN_ssm, self).__init__(init_cfg)
    ...
    def forward(self, inputs):
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            if 'scale_factor' in self.upsample_cfg:
                ...
                att_2x = F.interpolate(fpn_att_list[i], size=prev_shape, **self.upsample_cfg)
                att_insec = fpn_att_list[i - 1] * att_2x

                # get ROI of current attention map
                select_gate = att_insec

                laterals[i - 1] = laterals[i - 1] + select_gate * self.upsample_convs[i - 1](laterals[i])
