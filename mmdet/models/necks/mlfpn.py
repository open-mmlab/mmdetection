...
class BasicConv(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1,
        ...
        self.conv = ConvWS2d(in_planes, out_planes, kernel_size=kernel_size,
...
class TUM(nn.Module):
    def __init__(self, first_level=True, input_planes=128, is_smooth=True, side_channel=512, scales=6, ssd_style_tum = True):
       ...
       # print(self.base_list[1])
        if base_choice == 1:
            ...
            if self.backbone_choice == 'ResNet':
                ...
                self.shallow_out = 256
                self.deep_out = 512
            self.reduce = BasicConv(self.shallow_in, self.shallow_out, kernel_size=3, stride=1, padding=1)
            self.up_reduce = BasicConv(self.deep_in, self.deep_out, kernel_size=1, stride=1)
            ...
            if i == 0:
                setattr(self,
                        ...
                        TUM(first_level=True,
                            ...
                            side_channel=512,
                            ssd_style_tum=self.ssd_style_tum)) #side channel isn't fixed.
            else:
                setattr(self,
                        'unet{}'.format(i+1),
                ...
                if self.backbone_choice == 'ResNet':
                    base_feature = torch.cat(
                        (self.reduce(input[self.base_list[0]-1]),
                         F.interpolate(self.up_reduce(input[self.base_list[1]-1]), scale_factor=2, mode='nearest')), 1
                    )
                else:
                    base_feature = torch.cat(
                        (self.reduce(input[0]),
                         F.interpolate(self.up_reduce(input[1]), scale_factor=2, mode='nearest')), 1
                    )
