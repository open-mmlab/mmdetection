...
- Change to use FP16 utils in MMCV for FP16 training (#3766, #3822): To unify the FP16 training of OpenMMLab projects, the functions for FP16 training in `mmdet.core.fp16` are moved to `mmcv.runner`, including `force_fp32`, `auto_fp16`, `wrap_fp16_model`, and `Fp16OptimizerHook`. To help the users migrate the API calling from `mmdet.core.fp16` to `mmcv.runner`, warnings will show when the users are still using the FP16 modules from `mmdet.core.fp16` (#3822). The Deprecated warnings and modules will be removed in 2.8.0.
...
- Refactor `get_subset_by_classes` in dataloader for training with empty-GT images (#3695):
