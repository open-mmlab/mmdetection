...
`default_hooks` is a dict of hook configs. `default_hooks` are the hooks that must be required at runtime. They have default priority which should not be modified. If not set, runner will use the default values. To disable a default hook, users can set its config to `None`.
...
There are 4 basic component types under `config/_base_`, dataset, model, schedule, and default_runtime.
