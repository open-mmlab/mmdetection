...
4. The file locations of Transformer modules were rearranged. We made a new folder `transformer` in `mmdet/models/layers`. The Transformer modules of each DETR have been moved to `xxx_layers.py` in `transformer` folder, and other modules in original `transformer.py` have been moved to `utils.py` in `transformer` folder. They all can be imported from both `mmdet/models/layers` and `mmdet/models/layers/transformer`.
