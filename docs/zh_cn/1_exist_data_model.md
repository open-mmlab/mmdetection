...
**请记住：如果使用不是默认批次大小为`16`的配置文件，请检查配置文件中的底部，会有 `auto_scale_lr_config.default_batch_size`，并且当默认初始学习率不是 `0.01` 的配置文件中还可以找到`auto_scale_lr_config.default_initial_lr`。另外，如果想使用自动缩放学习率的功能，请不要修改这些值。**
