...
- Compatibility issue between MMCV and MMDetection; "ConvWS is already registered in conv layer"; "AssertionError: MMCV==xxx is used but incompatible. Please install mmcv>=xxx, <=xxx."
  ...
  Please install the correct version of MMCV for the version of your MMDetection following the [installation instruction](https://mmdetection.readthedocs.io/en/latest/install.html#install-mmdetection).
...
- "invalid device function" or "no kernel image is available for execution".
    ...
    You may need to set `TORCH_CUDA_ARCH_LIST` to reinstall MMCV, i.e. run `TORCH_CUDA_ARCH_LIST=7.0 pip install mmcv-full` to build MMCV.
