...
- `--images_out_dir`: If specified, detection results will be plotted on the images and saved to the specified directory. It is only applicable to single GPU testing and used for debugging and visualization. You **don't** need a GUI available in your environment for using this option.

If you would like to evaluate the dataset, do not specify `--show` nor `--images_out_dir` at the same time.
...
python tools/test.py configs/faster_rcnn_r50_fpn_1x.py \
    checkpoints/faster_rcnn_r50_fpn_1x_20181010-3d1b3351.pth \
    --images_out_dir faster_rcnn_r50_fpn_1x_results
```
